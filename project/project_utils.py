...
def plot_map(NOMINAL_GATES, NOMINAL_OBSTACLES):
    """
    Plots the 2D (XY-plane) positions of nominal gates and obstacles.

    Parameters:
    - NOMINAL_GATES: list or np.ndarray of shape (N, 3)
    - NOMINAL_OBSTACLES: list or np.ndarray of shape (M, 3)
    """
    ...
    # Plot gates
    ax.scatter(gates[:, 0], gates[:, 1], c='blue', marker='x', s=100, label='Gates')


    # Plot obstacles
    ax.scatter(obstacles[:, 0], obstacles[:, 1], c='red', marker='o', s=60, label='Obstacles')

    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_title('2D Map: Nominal Gates and Obstacles')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    plt.show()
    
def plot_full_map_and_trajectory_2d(NOMINAL_GATES,
                                    NOMINAL_OBSTACLES,
                                    waypoints,
                                    ref_x,
                                    ref_y):
    """
    Plots a 2D map showing gates, obstacles, and trajectory path.

    Parameters:
    - NOMINAL_GATES: list or np.ndarray of shape (N, 3)
    - NOMINAL_OBSTACLES: list or np.ndarray of shape (M, 3)
    - waypoints: np.ndarray of shape (W, 3), used for labeling waypoints if needed
    - ref_x: list or np.ndarray of x coordinates of the trajectory
    - ref_y: list or np.ndarray of y coordinates of the trajectory
    """
    gates = np.array(NOMINAL_GATES)
    obstacles = np.array(NOMINAL_OBSTACLES)
    ...
    # Plot trajectory
    ax.plot(ref_x, ref_y, color='green', linewidth=2, label='Trajectory')
    ...
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_title('2D Map with Gates, Obstacles, and Trajectory')
    ax.legend()
    ax.grid(True)
    plt.tight_layout()
    plt.show()
